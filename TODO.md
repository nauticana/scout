...
|---|---|---|---|
| C5 | Retrofit prompt inheritance onto `config_scope_binding`: `prompt_baseline`, `tenant_prompt_default`, and `agent_prompt_override` become `prompt_section` bindings, `PromptSourceLevel` is deleted, `CompiledPromptSection` carries its `Provenance`. Clean break, no shim, no data migration. | MED | — |
| B1a | Per-principal concurrency ceiling on `TenantWeightPolicy`, and `isolation.BudgetLedger` reserving per principal inside the tenant envelope. Closes D1's sub-turn settlement against the delegating agent and the last verification gate (a delegation chain exhausting its budget at hop two settles per hop). | MED | — |
| V3 | One decision record per decision under turn replay (records are append-only, so a redelivered turn can write a second row), plus an integration harness asserting a gap-free chain from ingress to terminal state. | MED | — |
...
| X12a | Cross-process turn replies and cancellation. `MemoryReplyHub` and `MemoryTurnCanceller` are in-process, but the runtime runs in a worker process. Needs either a publisher over a shared transport or a durable poll API over the retained frames. | MED | choice of transport (keel realtime/pub-sub vs. a frame table) |
| X12b | Carry the acting principal through `turn_queue`. A queue-delivered turn acts as its agent at the pinned version; `OnBehalfOf`, delegation bounds, the work item, and an authority chain are dropped, so delegated turns cannot cross the queue. | MED | — |
| X12c | Mirror a runtime turn's result and error into `conversation_turn_detail`. The history row is written at open; the terminal payload still lives only on `conversation_turn`. | LOW | — |
