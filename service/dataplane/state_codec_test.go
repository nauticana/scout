...
func newObjectStateStore(storage *fake.ObjectStorage) *ObjectStateStore {
	storage.Name = "sessions"
	return &ObjectStateStore{Storage: storage, KeyPrefix: "scout", MaxBytes: 1 << 20}
}
...
func TestObjectStateStoreRoundTripUsesContentAddressedKeys(t *testing.T) {
	...
	if _, ok := storage.Payload(wantKey); !ok {
...
func TestObjectStateStoreHydrateFailsClosedOnDigestMismatch(t *testing.T) {
	...
	storage.Overwrite(key, []byte("tampered"))
	got, err := store.Hydrate(context.Background(), ref)
...
func TestObjectStateStoreEnforcesBounds(t *testing.T) {
	store := &ObjectStateStore{Storage: &fake.ObjectStorage{Name: "b"}, MaxBytes: 4}
	...
	unbounded := &ObjectStateStore{Storage: &fake.ObjectStorage{Name: "b"}}
	...
	storage := &fake.ObjectStorage{Name: "b"}
	small := &ObjectStateStore{Storage: storage, MaxBytes: 4}
	ref, err := small.Dehydrate(context.Background(), "n", []byte("1234"))
	if err != nil {
		t.Fatal(err)
	}
	storage.Overwrite("n/"+ref.Digest, []byte("123456"))
